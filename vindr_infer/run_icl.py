#!/usr/bin/env python3
"""
VinDr-Mammo VLM 推理 - ICL Prompt (2 few-shot: 1 正常 + 1 Mass)

用法:  python vindr_infer/run_icl.py
流程:  1) build_mllm.py --prompt icl   注入 prompt -> mllm_infer.json (覆盖式, 每条3图)
       2) llamafactory-cli train infer.yaml  批量推理 (自带 tqdm 进度条, 实时流式输出)
       3) 解析 predict_results.json, 输出统计
日志:  outputs/icl/infer.log  (完整推理输出, 含进度条记录)
...
def main():
    t0 = time.time()
    print("=" * 55)
    print(f"VinDr-Mammo 推理 - {PROMPT.upper()} Prompt (2 few-shot)")
    print(f"模型: Qwen3.5-4B | 数据: test_raw.json (4000条, 每条3图) | 输出: {OUTPUT}")
    print("=" * 55)
    run_build()
    run_infer()
    show_stats()

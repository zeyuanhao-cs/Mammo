#!/usr/bin/env python3
"""
VinDr-Mammo VLM 推理 - Direct Prompt (直接预测, 无 few-shot)

用法:  python vindr_infer/run_direct.py
流程:  1) build_mllm.py --prompt direct   注入 prompt -> mllm_infer.json (覆盖式)
       2) llamafactory-cli train infer.yaml  批量推理 (自带 tqdm 进度条, 实时流式输出)
       3) 解析 predict_results.json, 输出统计
日志:  outputs/direct/infer.log  (完整推理输出, 含进度条记录)
...
def main():
    t0 = time.time()
    print("=" * 55)
    print(f"VinDr-Mammo 推理 - {PROMPT.upper()} Prompt")
